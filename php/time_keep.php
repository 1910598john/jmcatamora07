...
if (isset($_POST['serial_id']) && isset($_POST['machine_id'])) {
    searchStaff($conn, $_POST['machine_id'], $_POST['serial_id']);
}
...
function searchMachineID($conn, $machineID, $serial){ 
    ...
    if ($result->num_rows > 0) {
        ...
        searchStaff($conn, $machineID, $serial);
    }
}

$missed_out = false;

function missed_time_out($conn, $serial, $company_id, $name, $class, $pos, $dept, $contact, $date, $branch) {
    ...
    $stmt->bind_param("ississssss", $company_id, $name, $class, $serial, $pos, $dept, $contact, $message, $date, $branch);
    $stmt->execute();
}

function searchStaff($conn, $machineID, $serial) {
    $sql = "SELECT name, class, position, department, date, contact_number, company_id, branch, status FROM staffs WHERE serialnumber = '$serial' AND branch = '$machineID'";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row['status'] == 'IN') {
            if ($row['date'] != date('Y-m-d')) {
                missed_time_out($conn, $serial, $row['company_id'], $row['name'], $row['class'], $row['position'], $row['department'], $row['contact_number'], $row['date'], $machineID);
                $status = 'IN';
            } else {
                $status = 'OUT';
            }
        ...
        if (isset($_POST['serialnumber'], $_POST['name'], $_POST['pos'], $_POST['dept'], $_POST['status']/* ,$_SESSION['companyid'] */) || isset($_SESSION['details'])) {
            ...
            if (isset($_POST['timeout'])) {
                ...
                $stmt->bind_param("sssississ", $name, $pos, $dept, $serialnumber, $_POST['timeout'], $status, $company_id, $_POST['date'], $machineID);
            } else {
                // Use prepared statement to prevent SQL injection
                $stmt = $conn->prepare("INSERT INTO attendance (name, position, department, serialnumber, time_logs, status, company_id, date, branch)
                VALUES (?, ?, ?, ?, NOW(), ?, ?, NOW(), ?)");
            
                $stmt->bind_param("sssisis", $name, $pos, $dept, $serialnumber, $status, $company_id, $machineID);
            }
            
            if ($stmt->execute()) {
                
                UPDATESTAFFSTATUS($conn, $serialnumber, $company_id, $status, $machineID);
                UPDATESTAFF($conn, $serialnumber, $company_id, $machineID);

                $Class = fetchClass($conn, $company_id, $serialnumber);
            
                if ($status == "OUT") {
                    //calculateDaysWorked($conn, $serialnumber, $company_id);
                    $in_log = getLastInLog($serialnumber, $company_id, $conn, $machineID);
                    ...
                    $sameday = checkIfSameDayST($conn, $serialnumber, $company_id, $machineID);
                
                    $totalWorkedToday = fetchHourWorkedToday($conn, $company_id, $serialnumber, $machineID);
                    ...
                    if (isset($_POST['timeout'])) {
                        ...
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param("ssissiiss", $name, $class, $company_id, $totalHoursWorked, $in_log, $serialnumber, $onLeave, $paid_status, $machineID);
                    }
                    ...
                    if ($in < $clock_in_sched) {
                      ...
                      early_in_app($conn, $company_id, $serialnumber, $machineID, $name, $pos, date("Y-m-d H:i:s"), $minutes, $early_in_pay);
    ...
    $sql = "SELECT time_logs FROM attendance WHERE company_id = '$company_id' AND serialnumber = '$serial' AND status = 'IN' AND branch = '$branch' AND DATE(date) = CURDATE() ORDER BY id DESC LIMIT 1";
    $result = $conn->query($sql);
    ...
    if (isset($_POST['timeout'])) {
        $sql = "SELECT hours_worked FROM staffs_trail WHERE serialnumber = '$serialnumber' AND branch = '$branch' AND company_id = '$company_id' AND DATE(date) = '". $_POST['date'] . "'";
        
    } else {
        $sql = "SELECT hours_worked FROM staffs_trail WHERE serialnumber = '$serialnumber' AND branch = '$branch' AND company_id = '$company_id' AND DATE(date) = CURDATE()";
    }
    ...
    if ($isSameDay2) {
        $sql = "UPDATE staffs_trail SET total_hours = total_hours + $hour_worked_today, ot_total = '$ot', ut_total = '$ut', ot_mins = '$ot_mins', ut_mins = '$ut_mins', late_mins = $penalty WHERE company_id = '$company_id' AND branch = '$branch' AND serialnumber = '$serial' AND id = '$id'";
    
    } else {
        $sql = "UPDATE staffs_trail SET total_hours = total_hours + $hour_worked_today, ot_total = '$ot', ut_total = '$ut', ot_mins = '$ot_mins', ut_mins = '$ut_mins', late_mins = '$late_mins' WHERE company_id = '$company_id' AND branch = '$branch' AND serialnumber = '$serial' AND id = '$id'";
    }
